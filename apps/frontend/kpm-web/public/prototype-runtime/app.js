...
const buttonPermissionRegistry = [
  ...
  { key: 'button:project-skus:save', name: '项目详情 / 保存 SKU', target: '保存 SKU', location: '项目详情' },
  { key: 'button:project-skus:delete', name: '项目详情 / 删除 SKU', target: '删除 SKU', location: '项目详情' },
  ...
  { key: 'button:order-statuses:create', name: '订单状态配置 / 新增', target: '新增', location: '订单状态配置' },
  { key: 'button:order-statuses:edit', name: '订单状态配置 / 编辑', target: '编辑', location: '订单状态配置' },
  { key: 'button:order-statuses:toggle', name: '订单状态配置 / 启用或停用', target: '启用 / 停用', location: '订单状态配置' },
  { key: 'button:order-statuses:delete', name: '订单状态配置 / 删除', target: '删除', location: '订单状态配置' },
...
const orderStatusOptions = [
  { name: '已创建', active: true, semantic: 'DEFAULT' },
  { name: '生产中', active: true, semantic: 'IN_PRODUCTION' },
  { name: '已发货', active: true, semantic: 'SHIPPED' },
  { name: '已收货', active: true, semantic: 'RECEIVED' },
  { name: '已完成', active: true, semantic: 'COMPLETED' },
];
...
let selectedCustomerName = customers[0]?.name || '';
...
let projectSkuModalOpen = false;
let projectSkuEditIndex = null;
...
let selectedSupportCustomer = customers[0]?.name || '';
let supportCustomerKeyword = '';
...
let sidebarCollapsed = localStorage.getItem('kpm.sidebarCollapsed') === 'true';
let taskFormState = {
...
function normalizeBackendProject(project = {}) {
  return {
    ...
    skus: asArray(project.skus).map((sku) => ({
      id: sku.id,
      projectId: sku.projectId,
      wholeMachinePartNumber: sku.wholeMachinePartNumber || sku.whole_machine_part_number || '',
      configurationName: sku.configurationName || sku.configuration_name || '',
      memoryType: sku.memoryType || sku.memory_type || '',
...
function normalizeBackendOrder(order = {}) {
  const skuSnapshot = parseJsonObject(order.skuSnapshot);
  return {
    id: order.id,
    type: order.type || order.orderType || '正式订单',
    status: order.status || activeOrderStatuses()[0] || '已创建',
    ...
    skuId: order.skuId || '',
    skuSnapshot,
    wholeMachinePartNumber: order.wholeMachinePartNumber || skuSnapshot.wholeMachinePartNumber || '',
    configurationName: order.configurationName || skuSnapshot.configurationName || '',
    memoryType: order.memoryType || skuSnapshot.memoryType || '',
    quantity: Number(order.quantity || 0),
    specification: order.specification || '',
    expectedShipDate: apiDate(order.expectedShipDate, prototypeToday),
    plannedShipDate: apiDate(order.plannedShipDate, ''),
    actualShipDate: apiDate(order.actualShipDate, ''),
...
function hydrateOptionsFromBootstrap(bootstrap = {}) {
  ...
  replaceArray(orderStatusOptions, enumOptionObjects(enumItems, 'order_status', orderStatusOptions));
...
function applyPrototypeSnapshot(snapshot = {}) {
  ...
  replaceArray(orderStatusOptions, asArray(snapshot.orderStatusOptions));
...
function salesUsers(selectedNames = []) {
  const selected = new Set(selectedNames.filter(Boolean));
  const scoped = resourceData.users.filter((user) => (user.status === '启用' && user.departments.includes('销售部')) || selected.has(user.name));
  return scoped.length ? scoped : activeUsers();
}

function supportUsers(selectedNames = []) {
  const selected = new Set(selectedNames.filter(Boolean));
  const scoped = resourceData.users.filter((user) => (user.status === '启用' && user.departments.includes('技术支持部')) || selected.has(user.name));
  return scoped.length ? scoped : activeUsers();
}
...
function searchablePeopleInput(id, selectedPeople = [], placeholder = '搜索姓名，多个用逗号分隔', candidateNames = personCandidateNames(selectedPeople)) {
  ...
  return `
    <div class="people-picker">
      ...
      ${renderNameDatalist(`${id}-options`, uniqueCandidates)}
      <div class="people-picker-options" data-people-options-for="${id}">
        ${uniqueCandidates.slice(0, 12).map((name) => `<button type="button" class="people-option" data-action="append-people-choice" data-people-target="${id}" data-person-name="${escapeAttr(name)}">${name}</button>`).join('') || '<small class="field-hint">暂无可选人员，请先在用户管理中创建并启用用户。</small>'}
      </div>
    </div>
  `;
}
...
const actionPermissionMap = {
  ...
  'save-project-sku': 'button:project-skus:save',
  'delete-project-sku': 'button:project-skus:delete',
  ...
  'toggle-order-status-option': 'button:resources:edit',
...
function applySidebarState() {
  ...
  localStorage.setItem('kpm.sidebarCollapsed', String(sidebarCollapsed));
}
...
function activeOrderStatuses() {
  return configuredEnumNames(orderStatusOptions);
}

function orderStatusOptionsForValue(currentValue) {
  return configuredEnumNames(orderStatusOptions, currentValue, ['已创建']);
}

function orderStatusSemantic(statusName) {
  return orderStatusOptions.find((status) => status.name === statusName)?.semantic || 'DEFAULT';
}

function projectCustomerStatusForOrderType(orderType) {
  const configured = orderTypeOptions.find((type) => type.name === orderType || type.value === orderType);
  if (configured?.semantic && activeCustomerProjectStatuses().includes(configured.semantic)) return configured.semantic;
  const mapping = {
    样品订单: '样机测试',
    预订单: '商机发掘',
    正式订单: '订单冲刺',
...
function renderProjectSkuModal(project) {
    ...
    <div class="modal-backdrop" data-modal-key="project-sku">
      <section class="modal-card form-modal wide">
        <div class="panel-head">
          <div>
            <h2>${project.externalName} SKU 管理</h2>
            <p>订单下单时选择 SKU，并把当时的 SKU 信息保存成订单快照。</p>
          </div>
          <button class="ghost-btn" data-action="open-project-sku-create">新增 SKU</button>
        </div>
        ${editingSku || projectSkuEditIndex === 'new' ? `
          <div class="sub-panel">
            <div class="form-grid">
              <div class="form-field">
                <label>整机料号</label>
                <input id="project-sku-part-number" class="text-input" maxlength="120" value="${editingSku?.wholeMachinePartNumber || ''}" placeholder="例如 K1352-4G64G-DUAL" />
              </div>
              <div class="form-field">
                <label>配置名称</label>
                <input id="project-sku-config-name" class="text-input" maxlength="160" value="${editingSku?.configurationName || ''}" placeholder="例如 P8 dual / 4GB+64GB / 双屏" />
              </div>
              <div class="form-field">
                <label>内存类型</label>
                <input id="project-sku-memory-type" class="text-input" maxlength="120" value="${editingSku?.memoryType || ''}" placeholder="例如 LPDDR4 + eMMC" />
              </div>
              <div class="form-field">
                <label>状态</label>
                <select id="project-sku-active" class="select-input">
                  <option ${editingSku?.active !== false ? 'selected' : ''}>启用</option>
                  <option ${editingSku?.active === false ? 'selected' : ''}>停用</option>
                </select>
              </div>
            </div>
            <div class="customer-form-actions">
              <button class="muted-btn" data-action="cancel-project-sku-edit">取消编辑</button>
              <button class="primary-btn" data-action="save-project-sku">${editingSku ? '保存 SKU' : '新增 SKU'}</button>
            </div>
          </div>
        ` : ''}
        <div class="table-wrap">
          <table>
            <thead><tr><th>整机料号</th><th>配置名称</th><th>内存类型</th><th>状态</th><th></th></tr></thead>
            <tbody>
              ${asArray(project.skus).map((sku, index) => `
                <tr>
                  <td>${sku.wholeMachinePartNumber}</td>
                  <td>${sku.configurationName}</td>
        ...
        <div class="customer-form-actions">
          <button class="primary-btn" data-action="close-project-sku-modal">关闭</button>
        </div>
      </section>
    </div>
  `;
      ...
      <div class="detail-side">
        ...
        <section class="info-card">
          <div class="panel-head compact-head">
            <div>
              <h3>SKU 管理</h3>
              <p>${asArray(project.skus).length} 个 SKU</p>
            </div>
            <button class="ghost-btn" data-action="open-project-sku-modal">维护</button>
          </div>
          <p>维护整机料号、配置名称和内存类型；订单会保存下单时的 SKU 快照。</p>
        </section>
    ...
    ${taskFormOpen ? renderFormModal(`
      <div class="panel task-form-panel">
        <div class="form-grid">
          ...
          <div class="form-field">
            <label>适用客户</label>
            <input id="task-customer" class="text-input" list="task-customer-options" placeholder="中性 / 适用于所有客户" />
            ${renderNameDatalist('task-customer-options', customers.map((customer) => customer.name))}
            <small class="field-hint">不选择客户时，任务默认为“中性”，适用于所有客户。</small>
          </div>
          <div class="form-field">
      ...
      <div class="detail-main">
        <section class="panel">
          ...
          <div class="project-meta task-meta-grid">
            ...
            <div class="meta-item">
              <span>适用客户</span>
              <strong>${task.customerName || '中性 / 所有客户'}</strong>
            </div>
            <div class="meta-item">
              <span>来源</span>
        ...
        <section class="panel">
          ...
          <div class="form-grid task-edit-form">
            ...
            <div class="form-field">
              <label>适用客户</label>
              <input id="task-detail-customer" class="text-input" list="task-detail-customer-options" value="${task.customerName || ''}" placeholder="中性 / 适用于所有客户" />
              ${renderNameDatalist('task-detail-customer-options', customers.map((customer) => customer.name))}
            </div>
            <div class="form-field">
              <label>执行者</label>
...
function projectSkus(project) {
  return asArray(project?.skus).filter((sku) => sku.active !== false);
}

function orderSkuOptions(projectNameOrId, currentSkuId = '') {
  const project = backendProjectByName(projectNameOrId) || projects.find((item) => item.id === projectNameOrId) || projects[0];
  const skus = projectSkus(project);
  if (currentSkuId && !skus.some((sku) => sku.id === currentSkuId)) {
    const snapshotOrder = orders.find((order) => order.skuId === currentSkuId);
    if (snapshotOrder) {
      skus.push({
        id: currentSkuId,
        wholeMachinePartNumber: snapshotOrder.wholeMachinePartNumber,
        configurationName: snapshotOrder.configurationName,
        memoryType: snapshotOrder.memoryType,
        active: true,
      });
    }
  }
...
function skuDisplayName(sku = {}) {
  return [sku.wholeMachinePartNumber, sku.configurationName, sku.memoryType].filter(Boolean).join(' · ') || sku.id || '-';
}

function orderSelectedSku(order) {
  return orderSkuOptions(order.projectName || order.projectId, order.skuId)[0] || {};
}
...
function collectOrderFormValue() {
  ...
  return {
    ...
    skuId: document.getElementById('order-sku').value,
    quantity,
...
function renderOrderForm(editingOrder = null, options = {}) {
  ...
  const skuOptions = orderSkuOptions(order.projectName || order.projectId, order.skuId);
  if (!order.skuId && skuOptions[0]) order.skuId = skuOptions[0].id;
  ...
  return `
    <div class="panel task-form-panel">
      ...
      <div class="form-grid">
        ...
        <div class="form-field">
          <label>订单状态</label>
          <select id="order-status" class="select-input">
            ${orderStatusOptionsForValue(order.status).map((status) => `<option ${order.status === status ? 'selected' : ''}>${status}</option>`).join('')}
          </select>
          <small class="field-hint">状态可在资源管理中配置；变为“已发货”时后端自动记录实际发货日期。</small>
        </div>
        <div class="form-field">
          <label>下单日期</label>
        ...
        <div class="form-field full">
          <label>SKU</label>
          <select id="order-sku" class="select-input">
            ${skuOptions.map((sku) => `<option value="${sku.id}" ${order.skuId === sku.id ? 'selected' : ''}>${skuDisplayName(sku)}</option>`).join('')}
          </select>
          ${skuOptions.length ? '' : '<small class="field-hint warning">当前项目还没有启用 SKU，请先在项目详情中维护 SKU。</small>'}
        </div>
        <div class="form-field">
          <label>数量</label>
...
function renderOrderDetailModal(order) {
  const sku = {
    id: order.skuId,
    wholeMachinePartNumber: order.wholeMachinePartNumber || order.skuSnapshot?.wholeMachinePartNumber,
    configurationName: order.configurationName || order.skuSnapshot?.configurationName,
    ...
    <div class="modal-backdrop" data-modal-key="order-detail">
      <section class="modal-card form-modal wide">
        <div class="panel-head">
          <div>
            <h2>${order.id} 订单详情</h2>
            <p>列表仅展示基础信息；SKU 快照、规格、交付日期等详细信息在此查看。</p>
          </div>
          <span class="badge ${orderStatusSemantic(order.status) === 'COMPLETED' ? 'done' : 'progress'}">${order.status || '-'}</span>
        </div>
        ${renderDetailFactGrid([
          { label: '订单类型', value: order.type },
          { label: '下单日期', value: order.orderDate },
          { label: '客户', value: order.customerName },
          { label: '项目', value: order.projectName },
          { label: '整机料号', value: sku.wholeMachinePartNumber },
          { label: '配置名称', value: sku.configurationName },
          { label: '内存类型', value: sku.memoryType },
          { label: '数量', value: order.quantity },
          { label: '期望发货日期', value: order.expectedShipDate },
          { label: '计划发货日期', value: order.plannedShipDate || '-' },
          { label: '实际发货日期', value: order.actualShipDate || '-' },
          { label: '软件版本号', value: order.softwareVersion || '-' },
          { label: '金额', value: formatMoney(order.amount, order.currency) },
        ...
        <div class="customer-form-actions">
          <button class="primary-btn" data-action="close-order-detail">关闭</button>
        </div>
      </section>
    </div>
  `;
...
function renderOrderManagement() {
  ...
  orderManagement.innerHTML = `
    ...
    ` : ''}
    ${orderDetailModalOpen && selectedDetailOrder ? renderOrderDetailModal(selectedDetailOrder) : ''}
  `;
}
...
function renderSupportAnalytics() {
  ...
  if (!customer) {
    ...
    supportCustomerKeyword = '';
    analyticsContent.innerHTML = `
      <section class="panel">
        <div class="panel-head">
          <div>
            <h2>技术支持情况</h2>
            <p>当前没有客户数据，请先在客户管理中录入客户。</p>
          </div>
        </div>
        <p class="empty-note">暂无客户可查询</p>
      </section>
    `;
    return;
  }
  selectedSupportCustomer = customer.name;
...
function resourceRowsForTab(tab) {
  ...
  if (tab === 'order-statuses') return orderStatusOptions;
...
function orderStatusUsageCount(statusName) {
  return orders.filter((order) => order.status === statusName).length;
}

function canDeleteOrderStatus(option) {
  return orderStatusUsageCount(option.name) === 0;
}

function canToggleOrderStatusOption(option) {
  return !option.active || orderStatusUsageCount(option.name) === 0;
}
...
function renameOrderStatusReferences(previousName, nextName) {
  if (previousName === nextName) return;
  orders.filter((order) => order.status === previousName).forEach((order) => {
    order.status = nextName;
  });
}
...
function resourceTitleMeta(tab) {
  const titleMap = {
    ...
    'order-statuses': ['订单状态配置', '维护订单生命周期状态，例如已创建、生产中、已发货、已收货、已完成'],
...
function renderResourceForm() {
  ...
  if (selectedResourceTab === 'order-statuses') {
    return `
      <div class="customer-form">
        <div class="form-grid">
          <div class="form-field">
            <label>订单状态名称</label>
            <input id="order-status-name" class="text-input" maxlength="80" value="${editingItem?.name || ''}" placeholder="例如 质检中" />
          </div>
          <div class="form-field">
            <label>状态语义</label>
            <select id="order-status-semantic" class="select-input">
              ${['DEFAULT', 'IN_PRODUCTION', 'SHIPPED', 'RECEIVED', 'COMPLETED', 'CUSTOM'].map((semantic) => `<option ${editingItem?.semantic === semantic ? 'selected' : ''}>${semantic}</option>`).join('')}
            </select>
          </div>
          <div class="form-field">
            <label>是否启用</label>
            <select id="order-status-active" class="select-input">
              <option ${editingItem?.active !== false ? 'selected' : ''}>启用</option>
              <option ${editingItem?.active === false ? 'selected' : ''}>停用</option>
            </select>
          </div>
        </div>
        <p class="field-hint">“SHIPPED”语义用于后端自动记录实际发货日期；“DEFAULT”作为新增订单默认状态。</p>
        <div class="customer-form-actions">
          <button class="muted-btn" data-action="cancel-resource-form">取消</button>
...
function renderResourceTable() {
  ...
  if (selectedResourceTab === 'order-statuses') {
    return `
      <div class="table-wrap">
        <table>
          <thead><tr><th>订单状态</th><th>状态语义</th><th>启用状态</th><th>订单引用数</th><th></th></tr></thead>
          <tbody>
            ${rows.map(({ item, index }) => `
              <tr>
                <td>${item.name}</td>
                <td>${item.semantic || 'CUSTOM'}</td>
                <td><span class="badge ${item.active ? 'done' : 'pending'}">${item.active ? '启用中' : '已停用'}</span></td>
...
function createProjectWizardDraft() {
  const managerAccount = activeUsers()[0]?.account || currentAccount;
  return {
    externalName: '',
    internalName: '',
    modelName: '',
    ...
    description: '',
    members: [{ userAccount: managerAccount, role: defaultProjectMemberRole() }],
    stageAssignments: {},
  };
}
...
function collectResourcePayload(tab, previousItem = {}) {
  ...
  return {
    ...
    name,
    value: name,
    semantic: tab === 'task-statuses' ? document.getElementById('task-status-semantic').value
      : tab === 'order-statuses' ? document.getElementById('order-status-semantic').value
        : previousItem.semantic,
    active,
...
function renderAfterBackendRefresh() {
  ...
  if (!selectedCustomerName && customers.length) selectedCustomerName = customers[0].name;
  if (selectedSupportCustomer && !customers.some((customer) => customer.name === selectedSupportCustomer)) selectedSupportCustomer = customers[0]?.name || '';
  if (!selectedSupportCustomer && customers.length) selectedSupportCustomer = customers[0].name;
  renderAllPrototypeSurfaces();
  if (view === 'project-detail') renderProjectDetail();
...
function validateOrderForm() {
  return validateRequiredFields([
    ...
    { id: 'order-sku', label: 'SKU（需选择当前项目已启用的 SKU）', valid: (value) => Boolean(orderSkuOptions(document.getElementById('order-project')?.value || '', String(value)).some((sku) => sku.id === String(value))) },
    ...
    { id: 'order-expected-ship-date', label: '期望发货日期', valid: requiredIsoDateValue, message: '期望发货日期必须是 YYYY-MM-DD 格式' },
...
function validateTaskCreateForm(mode) {
  if (mode === 'task') {
    return validatePeopleSelection('task-assignees', '执行者', { required: true })
      ...
      { id: 'task-customer', label: '适用客户', valid: (value) => !String(value || '').trim() || Boolean(backendCustomerByName(String(value).trim())), message: '适用客户必须从已有客户中选择；不选择则默认为中性任务' },
...
function writeConfirmationConfig(action, actionTarget, overrides = {}) {
  ...
  if (action === 'save-project-sku') return { ...config, kind: projectSkuEditIndex === null || projectSkuEditIndex === 'new' ? 'submit' : 'modify', title: projectSkuEditIndex === null || projectSkuEditIndex === 'new' ? '确认新增 SKU？' : '确认保存 SKU 修改？' };
  if (action === 'delete-project-sku') {
    const project = projects.find((item) => item.id === selectedProjectId);
    const sku = project?.skus?.[Number(actionTarget.dataset.skuIndex)];
    return { ...config, title: '确认删除 SKU？', meta: [`项目：${project?.externalName || '-'}`, `SKU：${sku?.wholeMachinePartNumber || '-'}`] };
  }
...
function preflightWriteAction(action, actionTarget) {
  ...
  if (action === 'save-project-sku') return validateProjectSkuForm();
...
async function saveTaskViaApi(mode) {
  ...
  if (mode === 'task') {
    ...
    payload = backendTaskPayload({}, {
      ...
      creator: currentUser,
      customerName: document.getElementById('task-customer').value.trim(),
...
async function saveTaskDetailViaApi() {
  ...
  if (!validatePeopleSelection('task-detail-assignees', '执行者', { required: true })
    || !validatePeopleSelection('task-detail-participants', '参与者')
    || !validateRequiredFields([{ id: 'task-detail-customer', label: '适用客户', valid: (value) => !String(value || '').trim() || Boolean(backendCustomerByName(String(value).trim())), message: '适用客户必须从已有客户中选择；不选择则默认为中性任务' }])) return;
  ...
  await kpmApi.put(`/api/tasks/${task.id}`, backendTaskPayload(task, {
    ...
    customerName: document.getElementById('task-detail-customer').value.trim(),
...
function collectProjectSkuPayload() {
  return {
    wholeMachinePartNumber: document.getElementById('project-sku-part-number').value.trim(),
    configurationName: document.getElementById('project-sku-config-name').value.trim(),
    memoryType: document.getElementById('project-sku-memory-type').value.trim(),
    active: document.getElementById('project-sku-active').value === '启用',
  };
}

function validateProjectSkuForm() {
  return validateRequiredFields([
    { id: 'project-sku-part-number', label: '整机料号', valid: requiredMaxLength(120), message: '整机料号必填，且不能超过 120 个字符' },
    { id: 'project-sku-config-name', label: '配置名称', valid: requiredMaxLength(160), message: '配置名称必填，且不能超过 160 个字符' },
    { id: 'project-sku-memory-type', label: '内存类型', valid: requiredMaxLength(120), message: '内存类型必填，且不能超过 120 个字符' },
  ]);
}

async function saveProjectSkuViaApi() {
  const project = projects.find((item) => item.id === selectedProjectId);
  if (!project || !validateProjectSkuForm()) return;
  const payload = collectProjectSkuPayload();
  const editingSku = projectSkuEditIndex === null || projectSkuEditIndex === 'new' ? null : project.skus?.[projectSkuEditIndex];
  if (editingSku?.id) {
    await kpmApi.put(`/api/projects/${project.id}/skus/${editingSku.id}`, payload);
  } else {
    await kpmApi.post(`/api/projects/${project.id}/skus`, payload);
  }
  projectSkuEditIndex = null;
  await refreshBusinessDataAfterWrite('project-sku-save');
}

async function deleteProjectSkuViaApi(actionTarget) {
  const project = projects.find((item) => item.id === selectedProjectId);
  const sku = project?.skus?.[Number(actionTarget.dataset.skuIndex)];
  if (!project || !sku?.id) return;
  await kpmApi.delete(`/api/projects/${project.id}/skus/${sku.id}`);
  projectSkuEditIndex = null;
  await refreshBusinessDataAfterWrite('project-sku-delete');
}
...
async function handleRealApiClick(action, actionTarget) {
  ...
  if (action === 'save-project-sku') return saveProjectSkuViaApi();
  if (action === 'delete-project-sku') return deleteProjectSkuViaApi(actionTarget);
  ...
  if (action === 'toggle-order-status-option') return toggleEnumViaApi(orderStatusOptions[Number(actionTarget.dataset.orderStatusIndex)], !orderStatusOptions[Number(actionTarget.dataset.orderStatusIndex)]?.active);
...
const realApiClickActions = new Set([
  ...
  'save-project-sku', 'delete-project-sku',
  ...
  'toggle-customer-status-option', 'toggle-customer-level-option', 'toggle-order-type-option', 'toggle-order-status-option', 'toggle-task-status-option'
]);
...
document.addEventListener('click', async (event) => {
  ...
  if (action === 'open-order-detail') {
    selectedOrderDetailId = event.target.dataset.orderId;
  ...
  if (action === 'save-task-detail') {
    ...
    task.customerName = document.getElementById('task-detail-customer').value.trim();
  ...
  if (action === 'open-project-sku-modal') {
    projectSkuModalOpen = true;
    projectSkuEditIndex = null;
    renderProjectDetail();
  }
  ...
  if (action === 'open-project-sku-create') {
    projectSkuEditIndex = 'new';
    renderProjectDetail();
  }
  if (action === 'edit-project-sku') {
    projectSkuEditIndex = Number(actionTarget.dataset.skuIndex);
    renderProjectDetail();
  }
  ...
  if (action === 'save-project-sku' && !window.__kpmRealApiMode) {
    const project = projects.find((item) => item.id === selectedProjectId);
    if (project && validateProjectSkuForm()) {
      const payload = collectProjectSkuPayload();
      project.skus ||= [];
  ...
  if (action === 'delete-project-sku' && !window.__kpmRealApiMode) {
    const project = projects.find((item) => item.id === selectedProjectId);
    project?.skus?.splice(Number(actionTarget.dataset.skuIndex), 1);
    projectSkuEditIndex = null;
    renderProjectDetail();
  }
  ...
  if (action === 'append-people-choice') {
    const input = document.getElementById(actionTarget.dataset.peopleTarget);
    const personName = actionTarget.dataset.personName;
    if (input && personName) {
      const current = normalizePersonList(input.value.split(/[、,，]/));
      input.value = [...new Set([...current, personName])].join('、');
  ...
  if (action === 'delete-resource') {
    ...
    const canDelete = selectedResourceTab === 'departments' ? canDeleteDepartment(resource)
      ...
      : selectedResourceTab === 'order-statuses' ? canDeleteOrderStatus(resource)
  ...
  if (action === 'save-resource') {
    ...
    if (selectedResourceTab === 'order-statuses') {
      const name = document.getElementById('order-status-name').value.trim();
      if (name) {
        nextItem = {
          name,
          semantic: document.getElementById('order-status-semantic').value,
          active: document.getElementById('order-status-active').value === '启用',
        };
      }
    }
    ...
    if (nextItem) {
      ...
      if (previousItem && selectedResourceTab === 'order-statuses' && previousItem.active && !nextItem.active && orderStatusUsageCount(previousItem.name) > 0) {
        nextItem.active = true;
      }
      ...
      if (resourceEditIndex === null) {
        ...
        if (selectedResourceTab === 'order-statuses') {
          renameOrderStatusReferences(previousItem.name, nextItem.name);
        }
  ...
  if (action === 'toggle-order-status-option') {
    const index = Number(event.target.dataset.orderStatusIndex);
    if (!canToggleOrderStatusOption(orderStatusOptions[index])) return;
    orderStatusOptions[index].active = !orderStatusOptions[index].active;
    renderResourceContent();
  }
...
document.addEventListener('change', async (event) => {
  ...
  if (event.target.id === 'order-project') {
    const skuSelect = document.getElementById('order-sku');
    if (skuSelect) {
      const options = orderSkuOptions(event.target.value);
      skuSelect.innerHTML = options.map((sku) => `<option value="${sku.id}">${skuDisplayName(sku)}</option>`).join('');
    }
  }
...
document.addEventListener('input', (event) => {
  if (event.target.closest?.('.people-picker')) {
    const input = event.target;
    const optionBox = document.querySelector(`[data-people-options-for="${input.id}"]`);
    if (optionBox) {
      const lastToken = normalizePersonList(String(input.value || '').split(/[、,，]/)).at(-1)?.toLowerCase() || '';
      optionBox.querySelectorAll('.people-option').forEach((button) => {
        button.classList.toggle('hidden', Boolean(lastToken) && !button.textContent.toLowerCase().includes(lastToken));
      });
    }
  }
